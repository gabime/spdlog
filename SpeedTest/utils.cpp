...
void prepare_logdir(std::wstring log_folder)
{
	std::wstring wsc;
#ifdef _WIN32
	wsc = L"del /F /Q ";wsc.append(log_folder); wsc.append(L"*");
	auto rv = _wsystem(wsc.c_str());
#else
	wsc = L"rm -f "; wsc.append(dir); wsc.append(L"*");
	auto rv = _wsystem(wsc.c_str());
#endif
    (void)rv;
}
