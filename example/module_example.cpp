...
// spdlog::mdc is exported by the module, but MSVC fails to emit the thread_local in
// mdc::get_context() from an imported declaration ("unresolved external symbol
// __tlregdtor"). Including the header as well works around it and is harmless elsewhere.
...
#if !defined(SPDLOG_USE_STD_FORMAT)  // fmt formatter, via the re-exported fmt names
template <>
...
#else  // std::format formatter - <format> is included by the standard library, not by us
template <>
