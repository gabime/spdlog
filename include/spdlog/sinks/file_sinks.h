...
namespace sinks
{
    ...
    protected:
        void _sink_it(const details::log_msg& msg) override
        {
            ...
            if (_force_flush) _file_helper.flush();
        }
    ...
    public:
        ...
        static filename_t calc_filename(const filename_t& filename, std::size_t index)
        {
            typename std::conditional<
                std::is_same<filename_t::value_type, char>::value,
                fmt::MemoryWriter,
    ...
    struct default_daily_file_name_calculator
    {
        ...
        static filename_t calc_filename(const filename_t& filename)
        {
            ...
            filename_t basename, ext;
            std::tie(basename, ext) = details::file_helper::split_by_extenstion(filename);
            std::conditional<std::is_same<filename_t::value_type, char>::value, fmt::MemoryWriter, fmt::WMemoryWriter>::
                type w;
            w.write(
                SPDLOG_FILENAME_T("{}_{:04d}-{:02d}-{:02d}_{:02d}-{:02d}{}"),
                basename,
                tm.tm_year + 1900,
                tm.tm_mon + 1,
                tm.tm_mday,
