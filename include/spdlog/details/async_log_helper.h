...
inline void spdlog::details::async_log_helper::flush(bool wait_for_q)
{
    ...
    if (wait_for_q) wait_empty_q(); // return when queue is empty
}
    ...
    const spdlog::log_clock::time_point& last_op_time)
{
    ...
    // spin upto 50 micros
    if (time_since_op <= microseconds(50)) return;

    // yield upto 150 micros
    if (time_since_op <= microseconds(100)) return std::this_thread::yield();

    // sleep for 20 ms upto 200 ms
    if (time_since_op <= milliseconds(200)) return details::os::sleep_for_millis(20);
