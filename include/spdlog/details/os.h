...
// folder separator
...
#else
            auto fd = fileno(f);
            if (fcntl(fd, F_SETFD, FD_CLOEXEC) == -1) throw spdlog_ex("fcntl with FD_CLOEXEC failed", errno);
#endif
        }
        ...
        inline bool fopen_s(FILE** fp, const filename_t& filename, const filename_t& mode)
        {
            ...
            *fp = fopen((filename.c_str()), mode.c_str());
            ...
            if (*fp != nullptr) prevent_child_fd(*fp);
#endif
            return *fp == nullptr;
        }
...
#endif
            if (rv == TIME_ZONE_ID_INVALID) throw spdlog::spdlog_ex("Failed getting timezone info. ", errno);
...
#define SPDLOG_FILENAME_T(s) s
    ...
    ((_POSIX_C_SOURCE >= 200112L) && !defined(_GNU_SOURCE)) // posix version
        ...
        inline bool is_color_terminal()
        {
            ...
            static const bool result = std::any_of(std::begin(Terms), std::end(Terms), [&](const char* term) {
