...
void PrintResults(const std::map<size_t, std::vector<uint64_t>>& threads_result, size_t total_us)
{
    ...
    std::cout << "[spdlog] worst: " << std::setw(10) << std::right << worst << "\tAvg: " << avg
              << "\tTotal: " << utils::format(total_us) << " us" << std::endl;
}
} // anonymous
...
int main(int argc, char** argv)
{
    ...
    uint64_t total_time_in_us = std::chrono::duration_cast<std::chrono::microseconds>(
